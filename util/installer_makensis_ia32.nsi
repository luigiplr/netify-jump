...
;Parse package.json
!ifdef WIN_PATHS
    !searchparse /file "..\package.json" '"version": "' JUMP_VERSION '",'
!else
    !searchparse /file "../package.json" '"version": "' JUMP_VERSION '",'
!endif
!searchreplace JUMP_VERSION_CLEAN "${JUMP_VERSION}" "-" ".0"
!ifdef WIN_PATHS
...
!define COMPANY_NAME "Luigi Poole"
!define PRODUCT_VERSION "${JUMP_VERSION}" 
!define APP_NAME "Netify Jump"


Name "${APP_NAME}"
Caption "${APP_NAME} ${JUMP_VERSION}"
...
!ifdef WIN_PATHS
    OutFile "..\dist\${APP_NAME}-${JUMP_VERSION}-Windows-ia32-Setup.exe"
!else
    OutFile "../dist/${APP_NAME}-${JUMP_VERSION}-Windows-ia32-Setup.exe"
!endif
...
!define MUI_FINISHPAGE_RUN "$INSTDIR\Netify Jump.exe"
...
LangString desktopShortcut ${LANG_Catalan} "Drecera d'escriJUMPori"
...
Section
    ...
    ;Add the files
    !ifdef WIN_PATHS
        File /r "..\dist\Netify Jump-win32-ia32\"
    !endif
...
Section
    ...
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\${APP_NAME}.lnk" "$INSTDIR\Netify Jump.exe" "" "$INSTDIR\resources\JUMP_icon.ico" "" "" "" "${APP_NAME} ${JUMP_VERSION}"
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\Uninstall ${APP_NAME}.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\resources\JUMP_icon.ico" "" "" "" "Uninstall ${APP_NAME}"
